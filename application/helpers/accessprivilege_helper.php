...
if( ! function_exists('accessprivilegeuserlevel')){
	function accessprivilegeuserlevel($userlevelid, $moduleid){
		...
		if($countListAccessPrivilege > 0){
			...
			//Language
			if(@$rsListAccessPrivilege[8]['access_privilege_status'] == 1 && $rsListAccessPrivilege[8]['module_privilege_active'] == 1) $language = 1;
			
			$arrPrivilege = array($list,$view,$add,$edit,$publish,$approve,$delete,$order,$language);
		} else {
			show_404();
		}
