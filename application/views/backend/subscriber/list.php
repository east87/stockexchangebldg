                             ...
                             <table class="m-datatable" id="html_table" width="100%">
                                 ...
                                 <tbody>
                                  ...
                                  <?php
                                    ...
                                    <tr>
                                      <td><?php echo $no;?></td>
                                    <td colspan="2"><?php echo $subscriber['email'];?></td>
                                      <td><?php echo tgl_indo($subscriber['subscriber_date']);?></td>
                                              
                                      <td>
                                         
                                          <?php if($approve){ ?>
                                                    <?php if($subscriber['subscriber_status'] == 1) {?>
                                                            <a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$subscriber['subscriber_id'];?>"><i class="la la-check-circle"></i></a> &nbsp; 
                                                    <?php } else { ?>
                                                            <a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$subscriber['subscriber_id'];?>"><i class="la la-power-off"></i></a> &nbsp;
                                                    <?php } ?>
                                            <?php } ?>
                                            <?php if($delete){ ?>
                                                    <a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $subscriber['email'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND.'/'.$controller;?>/delete/<?php echo $subscriber['subscriber_id'];?>';}"><span class="la la-trash"></span></a> &nbsp;
                                            <?php } ?>
                                      </td>
                                    </tr>
                                    <?php } ?>
