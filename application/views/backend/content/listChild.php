                                            ...
                                            <td>
                                                    <?php if($approve){ ?>
                                                            <?php if($content['row_active_status'] == 1) {?>
                                                                    <a class="btn-outline-success btn-sm" title="Click to Inactive" href="<?= BASE_URL_BACKEND.'/'.$controller;?>/activeChild/<?= $content['row_id'];?>"><i class="la la-check-circle"></i></a> 
                                                            <?php } else { ?>
                                                                    <a class="btn-outline-danger btn-sm" title="Click to Active" href="<?= BASE_URL_BACKEND.'/'.$controller;?>/activeChild/<?= $content['row_id'];?>"><i class="la la-power-off"></i></a>
                                                            <?php } ?>
                                                    <?php } ?>
                                                    <?php if($edit){ ?>
                                                            <a class="btn-outline-primary btn-sm" title="Click to Edit" href="<?= BASE_URL_BACKEND.'/'.$controller;?>/editChild/<?= $label_id;?>/<?= $content['row_id'];?>"><i class="la la-edit"></i></a>
                                                    <?php } ?>
                                                    <?php if($delete){ ?>
                                                            <a class="btn-outline-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?= $content['row_id'];?> ?'); if (answer){window.location = '<?= BASE_URL_BACKEND.'/'.$controller;?>/deleteChild/<?= $content['row_id'];?>';}"><span class="la la-trash"></span></a>
                                                    <?php } ?>
                                            </td>
                                        </tr>
