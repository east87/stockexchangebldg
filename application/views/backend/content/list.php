                                   ...
                                   <?php
                                    ...
                                    <tr>
                                        ...
                                        <?= @resultData($content['content'],$controller);?> 
                                         <?php if($order){ ?>
                                            <td><input type="text" class="form-control" style="text-align:center;" name="order[<?= $content['row_id'];?>]" size="1" maxlength="2" value="<?= $content['row_order'];?>"></td>
                                        <?php } ?>
                                       <td colspan="3">
                                                    <?php if($approve){ ?>
...
<script language="javascript">
	$(document).ready(function() {
		...
		$("#langid\\[\\]").change(function(e){
			...
			if(size == 3){
				window.location = '<?= BASE_URL_BACKEND.'/'.$controller;?>/editLang/'+val;
			} else if(size == 2){
				window.location = '<?=BASE_URL_BACKEND.'/'.$controller;?>/addLang/'+val;
			}
		});
